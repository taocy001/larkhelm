...
def _collect_commit_diff(cwd: str, since_sha: str) -> list[str]:
    """Return list of file paths changed in since_sha..HEAD. Returns [] on any failure."""
    import subprocess as _sp
    if not cwd or not since_sha:
        return []
    try:
        proc = _sp.run(
            ["git", "-C", cwd, "diff", "--name-only", f"{since_sha}..HEAD"],
        ...
        _debug_log(f"[WorkspaceFinalize] _collect_commit_diff failed: {_e}")
        return []
