...
def _cmd_commit(chat_id: str, msg_id: str = None):
    """/commit — rebase, squash, stage, and push with a conventional commit message."""
    from larkhelm.handlers._query import _do_query

    model = _get_chat_model(chat_id)

    def _git(args_str: str) -> str:
        out, _, _ = _run_shell(chat_id, f"git {args_str}")
        return out.strip()

    _git("fetch --quiet")
    branch = _git("branch --show-current") or "HEAD"
    status = _git("status -sb") or "(nothing to commit)"
    diff = _git("diff HEAD")
    diff_body = (diff or "(no staged/unstaged changes)")[:8000]
    ahead_raw = _git("rev-list @{u}..HEAD --count")
    try:
        ahead = int(ahead_raw)
    except ValueError:
        ahead = 0
    ahead_msgs = _git("log @{u}..HEAD --oneline") or "(none)"
    recent = _git("log @{u} --oneline -6") or "(none)"

    prompt = f"""## Context (pre-gathered)
- Branch: {branch}
- Remote sync status:
{status}
...
{ahead_msgs}
- Recent pushed commits (for message style reference):
{recent}
...
Analyse what changed across all staged content. Write a **single** English commit message:
...
"""
    ...
    _do_query(chat_id, prompt, model, user_msg_id=msg_id)
