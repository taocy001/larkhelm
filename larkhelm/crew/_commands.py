...
典型分配示例：
- 写代码 → claude；审查代码 → gemini
- 设计方案 → claude；调研竞品/文档 → gemini
- 多个并行调研子任务 → 各自用 gemini；汇总整合 → claude
- 分析设计（claude）→ 并行实现多模块（claude×N）→ 并行审查（gemini×N）→ 汇总（claude）
...
def _crew_plan(chat_id: str, requirement: str, cwd: str,
               max_agents: int, cancel_ev: threading.Event) -> CrewPlan | None:
...
def cmd_crew(chat_id: str, args_str: str, user_msg_id: str | None = None):
...
def cmd_dev(chat_id: str, args_str: str, user_msg_id: str | None = None):
...
def _run_generic_crew_inner(chat_id: str, requirement: str,
                             max_agents: int, total_timeout: int,
                             user_msg_id: str | None, crew_id: str):
...
def _run_dev_crew(chat_id: str, requirement: str, user_msg_id: str | None,
...
def _run_dev_crew_inner(chat_id: str, requirement: str, user_msg_id: str | None,
