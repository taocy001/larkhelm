...
Routing rules (priority high → low):
  ...
  3. user preference backend_id (chat_state) → registry.get(backend_id)
  4. config default_backend → orchestrator chain → first healthy enabled
...
def resolve_backend(
    ...
    # Rule 3: user preference (backend_id or model set via /model command)
    ...
    # Rule 4: config default_backend → orchestrator chain → first healthy enabled
